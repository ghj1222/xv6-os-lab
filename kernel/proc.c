...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  kuvmmap(p->kernel_pagetable, p->pagetable, 0, PGSIZE);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    sz1 = uvmdealloc(p->pagetable, sz, sz + n);
    kuvmunmap(p->kernel_pagetable, sz, sz1);
  }
  p->sz = sz1;
  return 0;
}
...
fork(void)
{
  ...
  if (kuvmmap(np->kernel_pagetable, np->pagetable, 0, p->sz) == 0)
  {
    freeproc(np);
    release(&np->lock);
    return -1;
